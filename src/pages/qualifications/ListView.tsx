            ...
            {isLoadingTable ? (
              ...
              filteredQualifications.map((qualification) => (
                <tr key={qualification.id} className={cn(
                  ...
                  <td className="px-6 py-4 whitespace-nowrap font-medium">{qualification.name}</td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${qualification.isTest
                      ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-800 dark:text-yellow-100'
                      : 'bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-100'
                    }`}>{qualification.isTest ? 'Test' : 'Production'}</span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{qualification.questions.length} questions</td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <button onClick={() => handleToggleActive(qualification.id)} className="flex items-center">
                      {qualification.active ? <ToggleRight className="w-6 h-6 text-green-500" /> : <ToggleLeft className="w-6 h-6 text-gray-400" />}
                    </button>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <Button variant="ghost" size="sm" onClick={() => handleEditQualification(qualification)}>
