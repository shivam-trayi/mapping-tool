...
interface QualificationMappingDataItem {
	...
	mapped: boolean; // current mapping
	oldMapped: boolean; // old mapping exists
	constantId: string; // input for new mapping
}
...
export const QualificationsMappingView: React.FC<QualificationsMappingViewProps> = ({
	...
	const handleSelectAll = (checked: boolean) => {
		...
		setSelectedItems(checked ? new Set(fetchedQualifications.map((item) => item.id)) : new Set());
	};
	...
	const handleSaveForReview = async () => {
		...
		const selectedData = Array.from(selectedItems).map((id) => {
			const item = fetchedQualifications.find((d) => d.id === id)!;
			return {
				qualification_id: Number(item.id),
		...
		try {
			...
			const response = (await dispatch(getAllQualMapping({ memberId: selectedCustomer })).unwrap()) as any;
			...
			setFetchedQualifications(
				apiData.map((item: any) => ({
					...
					constantId: item.member_qualification_id ? String(item.member_qualification_id) : "",
				}))
			);

			setReviewMappings(
				selectedData.map((d) => ({
					id: String(d.qualification_id),
	...
	return (
      ...
      <FileText className="w-4 h-4 mr-2" /> Question Mapping
				...
				<div className={cn("rounded-lg shadow border transition-colors", resolvedTheme === "dark" ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200")}>
							...
							<table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
								<thead className={cn("sticky top-0 z-10", resolvedTheme === "dark" ? "bg-gray-700 text-gray-200" : "bg-gray-50 text-gray-500")}>
									<tr>
										...
										<th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider">Enter Constant Id</th>
									</tr>
								</thead>
						...
						<div className={cn("p-4 border-t flex items-center justify-between sticky bottom-0 z-20 bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700")}>
							...
							<Button onClick={handleSaveForReview} disabled={selectedItems.size === 0 || isSaving} className="gradient-primary text-white hover:shadow-glow transition-all duration-300">
								{isSaving ? (
									...
									<>
										<Save className="w-4 h-4 mr-2" />Qualifications Save for Review ({selectedItems.size})
									</>
								)}
							</Button>
						</div>
