...
export const QualificationsMappingView: React.FC<QualificationsMappingViewProps> = ({
    ...
    // Mock data (no pagination now, using all qualifications directly)
    const qualificationMappingData: QualificationMappingData[] = qualifications.map((q, index) => ({
        id: q.id,
    ...
    const handleSelectAll = (checked: boolean) => {
        ...
        if (checked) {
            setSelectedItems(new Set(qualificationMappingData.map(item => item.id)));
        } else {
    ...
    return (
        ...
        >
            ...
            <div className={cn(
                ...
                {/* Footer */}
                <div className={cn(
                    "p-4 border-t flex items-center justify-between transition-colors sticky bottom-0 z-20",
