...
export const DemoPriorityMappingView: React.FC<DemoPriorityMappingViewProps> = ({
  ...
  // Mock data for demo
  const mockMappingData: MappingData[] = qualifications.map((q, index) => ({
    id: q.id,
    qualificationName: q.name,
    mapped: index % 3 === 0, // Some items are mapped
    constantId: index % 3 === 0 ? `CONST_${index + 1}` : '',
  }));

  const filteredData = mockMappingData.filter(item =>
  ...
  const handleSaveReview = () => {
    const selectedData = Array.from(selectedItems).map(id => {
      const item = mockMappingData.find(d => d.id === id);
      return {
        ...item,
    ...
    alert(`Saved ${selectedData.length} items for review`);
  };
    ...
    <div className={cn(
      ...
      <div className={cn(
        ...
        >
          ...
          Save Review ({selectedItems.size})
        </Button>
      </div>
    </div>
  </motion.div>
